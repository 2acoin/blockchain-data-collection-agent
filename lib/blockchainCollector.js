...
Self.prototype.getPoolChangesLite = function (currentBlock) {
  return new Promise((resolve, reject) => {
    this.emit('debug', 'Attempting to retrieve the transaction pool details...')
    this.daemon.getPoolChangesLite(currentBlock).then((result) => {
      return resolve(result)
    }).catch((error) => {
      return reject(new Error('Could not collect transaction pool details from daemon: ' + error.message))
    })
  })
}

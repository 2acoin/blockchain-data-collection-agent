...
const txnPoolDetailInsert = [
  'INSERT INTO `transaction_pool_detail` (',
  '`txnHash`, `extra`, `keyImage`, `keyOffsets`, `amount`, `inOut`',
  ') VALUES (?,?,?,?,?,?)'
].join('')
...
Self.prototype.saveTransactionPoolDetail = function (details) {
  return new Promise((resolve, reject) => {
    var queries = this.buildSaveTransactionPoolDetailQueries(details)
...
Self.prototype.buildSaveTransactionPoolDetailQueries = function (details) {
  var queries = []
  queries.push({ query: 'TRUNCATE `transaction_pool_detail`', args: [] })

  /* begin processing of all the transactions in the mempool */
  for (var i = 0; i < details.addedTxs.length; i++) {
    var transaction = details.addedTxs[i]
	var pubkey = transaction['transactionPrefixInfo.txPrefix'].extra.substr(2,64)

/* Don't process inputs at this time
    //process vIn details
    for (var x = 0; x < details.addedTxs[i]['transactionPrefixInfo.txPrefix'].vin.length; x++) {
      var vin = details.addedTxs[i]['transactionPrefixInfo.txPrefix'].vin[x]
	  if (vin.length !== 0 && transaction.length !== 0) {
		  var key_offset = JSON.stringify(vin.value.key_offsets)
     	  queries.push({ query: txnPoolDetailInsert,
			args: [
			  transaction['transactionPrefixInfo.txHash'],
			  vin.value.k_image,
			  key_offsets,
			  vin.value.amount,
			  1
...
*/
    /* process vOut details            */
    for (var y = 0; y < details.addedTxs[i]['transactionPrefixInfo.txPrefix'].vout.length; y++) {
      var vout = details.addedTxs[i]['transactionPrefixInfo.txPrefix'].vout[y]
      if (vout.length !== 0 && transaction.length !== 0) {
          queries.push({ query: txnPoolDetailInsert,
            args: [
              transaction['transactionPrefixInfo.txHash'],
              pubkey,
    ...
    for (var j = 0; j < queries.length; j++) {
      console.log('[ALERT]  The VIN/VOUT queries are --> ' + queries[j].query + ' Args = ' + queries[j].args)
    }
  }
